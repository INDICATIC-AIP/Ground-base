...
int ExpositionTime = 1; // Time of expostition
...
void MyClient::sendTemperatureAlert(double temperature)
{
    ...
    sprintf(emailCmd, "/home/indicatic-e1/Desktop/AutoRun/SendMail.sh \"%s\" \"%s\"", subject, message);
...
void MyClient::shutdownCamera()
{
    ...
    sprintf(shutdownCmd, "/home/indicatic-e1/Desktop/app/camera_on_off.sh off alpy");
    system(shutdownCmd);
        ...
        watchDevice("SX CCD SX-825", [this, &stopWatchingTemperature, &lastCaptureTime](INDI::BaseDevice device)
        {
            ...
            device.watchProperty("CCD_TEMPERATURE", [this, &stopWatchingTemperature, &lastCaptureTime](INDI::PropertyNumber property)
            {
                ...
                property.onUpdate([property, this, &stopWatchingTemperature, &lastCaptureTime]()
                {
                    ...
                    char tempCmd[256];
                    sprintf(tempCmd, "/home/indicatic-e1/Desktop/app/alpy_monitor.sh temperature_update %.2f >> /tmp/alpyMQTT.txt 2>&1", currentTemp);
                    system(tempCmd);
                    ...
                    if (mWarmupComplete && currentTemp < TargetedTemp && currentTime - lastCaptureTime >= Offset)
                    {
                        ...
                        char startCmd[256];
                        sprintf(startCmd, "/home/indicatic-e1/Desktop/app/alpy_monitor.sh capture_start %d >> /tmp/alpyMQTT.txt 2>&1", ExpositionTime);
                        system(startCmd);

                        takeExposure(ExpositionTime); //Exposure of ExpositionTime seconds
                        IDLog("Exposition of %d has been executed\n",ExpositionTime);
            ...
            device.watchProperty("CCD1", [this](INDI::PropertyBlob property)
            {
                ...
                snprintf(filePath, sizeof(filePath), "/home/indicatic-e1/Desktop/ASTRODEVICES/ALPYFILE/ccd_sx_825_%dsecondes_%s.fits", ExpositionTime, dateTime); //Path where are saved the capture.
                const char* filename = strrchr(filePath, '/');
                filename = filename ? filename + 1 : filePath;
                char captureCompleteCmd[512];
                // use mCurrentTemp (stored when temperature updates arrived)
                snprintf(captureCompleteCmd, sizeof(captureCompleteCmd), "/home/indicatic-e1/Desktop/app/alpy_monitor.sh capture_complete %s \"%s\" %.2f >> /tmp/alpyMQTT.txt 2>&1", filename, dateTime, mCurrentTemp);
